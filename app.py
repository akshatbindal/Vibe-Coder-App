...
try:
    ...
    if not google_api_key:
        error_messages.append("🔴 Google API Key not found in secrets.")
    else:
        genai.configure(api_key=google_api_key)
        model = genai.GenerativeModel(GEMINI_MODEL_NAME)
except KeyError:
     error_messages.append("🔴 `my_secrets.gemini_api_key` not found in Streamlit secrets.")
except Exception as e:
    error_messages.append(f"🔴 Failed to configure Google AI: {e}")

try:
    preview_api_endpoint_url = st.secrets["my_secrets"]["preview_api_endpoint"]
    if not preview_api_endpoint_url:
         error_messages.append("🔴 AWS Preview API Endpoint URL not found in secrets.")
except KeyError:
     error_messages.append("🔴 `my_secrets.preview_api_endpoint` not found in Streamlit secrets.")
except Exception as e:
     error_messages.append(f"🔴 Failed to read Preview API Endpoint from secrets: {e}")
...
with st.sidebar:
    ...
    if st.session_state.preview_api_endpoint:
        st.success("AWS Preview Endpoint Configured.", icon="☁️")
    else:
        st.error("AWS Preview Endpoint Missing in Secrets.", icon="⚠️")
...
# --- Live Preview Tab ---
    ...
    st.header("▶️ AWS Lambda Preview")
    st.divider()
    st.warning("⚠️ Preview executes AI-generated code in an AWS Lambda environment. Ensure code is safe and the Lambda role has minimal permissions.")
    ...
    if not api_endpoint:
        st.error("🔴 Preview API Endpoint not configured. Set `preview_api_endpoint` in Streamlit secrets.")
        ...
        if st.button(f"🚀 Run Preview for {selected_filename}", type="primary", use_container_width=True):
            ...
            with st.spinner("🚀 Calling AWS Lambda endpoint..."):
    ...
    if preview_error:
        ...
        st.error(preview_error, icon="🔥") # Use st.error to display the error message
        ...
        if last_preview_code:
         ...
         st.success("Preview loaded successfully from AWS Lambda.", icon="✅")
         ...
         if selected_filename and last_preview_code != current_saved_code:
              st.warning(f"Preview below is for the previously saved version of '{selected_filename}'. Save changes and run preview again for updates.", icon="⚠️")
